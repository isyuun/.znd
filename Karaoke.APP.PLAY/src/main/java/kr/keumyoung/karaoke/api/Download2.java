...
class Download2 extends Download implements _Const {
	...
	public void run() {
		...
		if (BuildConfig.DEBUG) Log.w(_toString(), getMethodName() + "[ST]" + type + ":" + url + "->" + path);

		// super.run();
		try {

			if (TextUtil.isNetworkUrl(mMp3)) {
				if (BuildConfig.DEBUG) Log.i(_toString(), "lyric.down() " + "[ST]" + type + ":" + url + "->" + path);
				LyricsUtil.down(url, path);
				if (BuildConfig.DEBUG) Log.i(_toString(), "lyric.down() " + "[ED]" + type + ":" + url + "->" + path);
			} else {
				down();
			}
			...
			if (BuildConfig.DEBUG) Log.w(_toString() + TAG_ERR,  "[NG]" + getMethodName() + type + ":" + url + "->" + path);
		...
		if (BuildConfig.DEBUG) Log.w(_toString(), getMethodName() + "[ED]" + type + ":" + url + "->" + path);
	}
